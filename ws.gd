...
signal connection_accepted(data)
...
func _run(_self):
	...
	while true:
		...
		tm += 0.1
		if tm>TIMEOUT:
			print("Timeout! Thats enough sonny boy")
	...
	for lin in data.split("\n"):
		if lin.find("HTTP/1.1 101")>-1:
			connection_ok = true
			print("Recv another header")
		# other headers can by checked here
		if lin.find("CONNA") > -1:
			var args = lin.split(lin[0])
			on_accept = int(args[1])
			for i in range(2,args.size()):
				var command = args[i].strip_edges()
				if command.begins_with("a:"):
					print("Adding command: ", command)
					pre_commands.append(command)
			accepted = true
	
	print ("parsed headers, connection result (should be true) ==> ", accepted);
...
func start(host,port,path=null):
	...
	self.port = port
	set_big_endian(true)
	print("IP Addresses: ", IP.get_local_addresses())
